...
namespace ImageLib
{
  ...
  inline std::unique_ptr<Image> downsample(const int w, const int h, const int nc, const uint8_t* data) {
    auto to_u8 = [](double v) -> uint8_t {
      if (v < 0)
        return 0;
      if (v > 255)
    ...
    int nw = w / 2;
    int nh = h / 2;
    std::unique_ptr<Image> ret = std::make_unique<Image>(nw, nh, nc);
    uint8_t* dst = ret->data();
    for (int row = 0; row < nh; ++row)
    {
      for (int col = 0; col < nw; ++col)
      {
        int i00 = (row * 2 + 0) * w * nc + (col * 2 + 0) * nc;
        int i10 = (row * 2 + 0) * w * nc + (col * 2 + 1) * nc;
        int i01 = (row * 2 + 1) * w * nc + (col * 2 + 0) * nc;
        int i11 = (row * 2 + 1) * w * nc + (col * 2 + 1) * nc;
        for (int comp = 0; comp < nc; ++comp) {
          double v = double(data[i00 + comp]) + data[i10 + comp] + data[i01 + comp] + data[i11 + comp];
          v *= 0.25;
          dst[row * nw * nc + col * nc + comp] = to_u8(v);
        }
      }
    }
    return ret;
